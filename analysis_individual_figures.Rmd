...
Finally, we replot the heatmap with the same order of gRNAs, only plotting cell-cycle related genes, and restricting to gRNAs found to be resistant in the pooled screens, adding cluster and energy distance to the row annotations.  
...
top_markers_rep <- table(markers$DE_gene_name[markers$FDR<=0.1&abs(markers$summary.AUC-0.5)>0.2&markers$DE_gene_name%in%cc_genes])
frequent_marker_genes <- names(top_markers_rep)[order(top_markers_rep)[1:min(length(top_markers_rep),20)]]
...
df_row_pathway <- data.frame(variant_class=validation_lib$variant_class[match(gRNAs,validation_lib$ID)],
                             cluster=factor(sapply(validation_lib$cluster[match(gRNAs,validation_lib$ID)],toString),
                                            levels=c("1","2","3","4")))
...
pheatmap(t(gRNA_pathway_mean[as.vector(order(df_row_pathway[rownames(gRNA_pathway_mean),]$cluster)),]),annotation_col=df_row_pathway,annotation_colors = colors_pathway_heatmap ,col=col_pathway,show_colnames = FALSE,cluster_cols=FALSE,cutree_cols=4,cellheight = 10,treeheight_row = 0,breaks=breaks_pathway)
```
```{r heatmap_pathway_scores_clustered}
df_row_pathway <- data.frame(variant_class=validation_lib$variant_class[match(gRNAs,validation_lib$ID)],
                             cluster=factor(sapply(validation_lib$cluster[match(gRNAs,validation_lib$ID)],toString),
                                            levels=c("1","2","3","4")))
rownames(df_row_pathway) <- gRNAs
...
breaks_pathway = c(seq(min_val, 0, 
                  ...
                  seq(max_val/nr_colors, 
                      max_val, length.out=floor(nr_colors/2)))
pheatmap(t(gRNA_pathway_mean[as.vector(order(df_row_pathway[rownames(gRNA_pathway_mean),]$cluster)),]),annotation_col=df_row_pathway,annotation_colors = colors_pathway_heatmap ,col=col_pathway,show_colnames = FALSE,cluster_cols=TRUE,cutree_cols=4,cellheight = 10,treeheight_row = 0,breaks=breaks_pathway)
```

...
df_logcounts <- data.frame(variant_class=validation_lib$variant_class[match(colnames(av_logcount_per_gRNA),validation_lib$ID)],
                           transcriptional_impact = sapply(validation_lib$transcriptional_impact_binary[match(colnames(av_logcount_per_gRNA),validation_lib$ID)],toString))
df_logcounts$transcriptional_impact[df_logcounts$transcriptional_impact=="TRUE"] <- "significant"
df_logcounts$transcriptional_impact[df_logcounts$transcriptional_impact=="FALSE"] <- "not significant"

rownames(df_logcounts) <- colnames(av_logcount_per_gRNA)
cols_logcounts <- list(variant_class=colourscheme_variant_class,
                       transcriptional_impact=c("significant"="black","not significant"="grey"))
...
Finally, we replot the heatmap with the same order of gRNAs, only plotting cell-cycle related genes, and restricting to gRNAs found to be resistant in the pooled screens and having a significant overall transcriptional effect.
...
top_markers_rep <- table(markers$DE_gene[markers$FDR<=0.1&abs(markers$lfc)>0.5&markers$DE_gene%in%cc_genes])
...
pheatmap(t(av_logcount_per_gRNA_scaled_ordered_sub),annotation_row = df_logcounts_sub[,c("variant_class","cluster","log2_ed")],annotation_colors = cols_logcounts_sub,treeheight_row = 0,
        ...
        cluster_rows=FALSE,cutree_cols=2,treeheight_col=0)
...
```{r}
sce_canonical_drug_resistance <- sce_logcounts_and_pathway_scores[,sce_logcounts_and_pathway_scores$variant_class_highest_impact=="canonical drug resistance"]
